/**
 * Export engines: PDF (jsPDF, vector — text stays selectable), DOCX (docx) and
 * iCal (.ics, hand-built RFC 5545). All three consume the same `ExportData`
 * so the printed routine, the Word table and the calendar always agree.
 */
...
export interface IcsOptions {
  /** First day of the semester (YYYY-MM-DD). Events start on/after this date. */
  semesterStart: string
  /** Last day of classes (YYYY-MM-DD, inclusive). */
  semesterEnd: string
  /** Minutes before each class for a reminder; 0 disables the alarm. */
  reminderMinutes?: number
}

const ICS_DAY: Record<DayName, string> = {
  Sat: 'SA',
  Sun: 'SU',
  Mon: 'MO',
  Tue: 'TU',
...
const JS_DAY: Record<DayName, number> = { Sun: 0, Mon: 1, Tue: 2, Wed: 3, Thu: 4, Fri: 5, Sat: 6 }

function pad(n: number): string {
  return n.toString().padStart(2, '0')
}

/** Floating local date-time (no TZ) so the calendar app uses the user's zone. */
function icsLocal(date: Date, minutes: number): string {
...
function firstOccurrence(from: Date, day: DayName): Date {
...
export function buildIcs(data: ExportData, opts: IcsOptions): string {
  const { profile, grid } = data
  const start = parseDate(opts.semesterStart)
  const end = parseDate(opts.semesterEnd)
  const until = `${end.getFullYear()}${pad(end.getMonth() + 1)}${pad(end.getDate())}T235959`
  const stamp = new Date()
    .toISOString()
    .replace(/[-:]/g, '')
    .replace(/\.\d{3}/, '')
  const reminder = opts.reminderMinutes ?? 10

  const lines: string[] = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//Faculty Routine Extractor//EN',
    'CALSCALE:GREGORIAN',
    'METHOD:PUBLISH',
    `X-WR-CALNAME:${escapeIcs(routineTitle(profile))}${profile.fullName ? ` – ${escapeIcs(profile.fullName)}` : ''}`,
  ]

  const sorted = [...grid.slots].sort((a, b) => JS_DAY[a.day] - JS_DAY[b.day] || a.startMin - b.startMin)
  sorted.forEach((s, i) => {
    const first = firstOccurrence(start, s.day)
    if (first > end) return
    const summary = `${chipLabel(s)}${s.room ? ` (${s.room})` : ''}`
    const desc = [
      `${s.type === 'lab' ? 'Lab' : 'Theory'} class · ${fullCourseCode(s)}`,
      s.room ? `Room: ${s.room}` : '',
      profile.fullName ? `Faculty: ${profile.fullName}` : '',
    ...
    lines.push(
      'BEGIN:VEVENT',
      `UID:${stamp}-${i}-${s.id}@faculty-routine-extractor`,
      `DTSTAMP:${stamp}`,
      `DTSTART:${icsLocal(first, s.startMin)}`,
      `DTEND:${icsLocal(first, s.endMin)}`,
      `RRULE:FREQ=WEEKLY;BYDAY=${ICS_DAY[s.day]};UNTIL=${until}`,
      `SUMMARY:${escapeIcs(summary)}`,
      `DESCRIPTION:${escapeIcs(desc)}`,
      ...(s.room
        ? [`LOCATION:${escapeIcs(`Room ${s.room}${profile.institution ? `, ${profile.institution}` : ''}`)}`]
        : []),
      `CATEGORIES:${s.type === 'lab' ? 'LAB' : 'THEORY'},CLASS`,
    )
    if (reminder > 0) {
      lines.push(
        'BEGIN:VALARM',
...
export function exportIcs(data: ExportData, opts: IcsOptions): void {
  const blob = new Blob([buildIcs(data, opts)], { type: 'text/calendar;charset=utf-8' })
  saveAs(blob, `${baseFileName(data.profile)}.ics`)
}
