...
import { computeWorkload } from '../../src/application'
import { creditPolicy, describeCourseLine, formatSections, type ScheduleSlot } from '../../src/domain'
...
describe('computeWorkload', () => {
  ...
  it('counts a section once even when it meets several times a week', () => {
    expect(
      computeWorkload([mk('CSE 443', '1', 'theory', 'Mon'), mk('CSE 443', '1', 'theory', 'Tue')])
        .totalCredits,
    ).toBe(3)
  })

  it('supports custom credit policies and ignores blank codes', () => {
    expect(
      computeWorkload([mk('CSE 443', '1', 'theory')], { policy: creditPolicy({ theory: 4 }) }).totalCredits,
    ).toBe(4)
    expect(computeWorkload([mk('', '1', 'theory')]).courses).toEqual([])
